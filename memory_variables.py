...
def get_soft_typed_var(string: str, line: int) -> Union[float, str, bool, list]:
    if string == "":
        ...
        return True
    elif string == ("False", "false"):
        return False
    elif string in _str:
        return _str[string]
    elif string in _nb:
        return _nb[string]
    elif string in _bool:
        return _bool[string]
    elif string in _list:
        return _list[string]
    else:
...
def set_var(var_name: str, value: Union[float, str, bool, list]) -> None:
    ...
    if isinstance(value, str):
        _str.update({var_name: value})
    if isinstance(value, bool):
        _bool.update({var_name: value})
...
def get_type(code: str, line: int) -> Optional[type[bool] | type[str] | type[float]]:
    ...
    if " or " in code or " nor " in code or " and " in code or " nand " in code or " xor " in code or " xnor " in code or "not " in code:
        ...
        if "-" in code or "*" in code or "/" in code or "^" in code or "%" in code:
            ...
            return type(get_soft_typed_var(no_space(code), line))
...
def quote_safe_no_space(string: str) -> str:
    return "".join(quote_safe_slice(string, " "))
    ...
    for char in code_line:
        ...
        if char == ")":
            ...
            if opened_parenthesis < 0:
                raise syntax_exception(code_line, line_nb)
            if opened_parenthesis == 0:
